...
//Herman Chanel(SC ORY)
//go 1k Subscribe:)
//Ayo Bangkit kan Sc ini
//semangat Ngab
//jangan menyerah:v
//selamat Remcode awkowko
const {
   WAConnection,
   MessageType,
   Presence,
...
limitawal = 9999 //Bebas Ganti Limit
memberlimit = 1
cr = '*Herman Botz*' 

/******** OWNER NUMBER**********/
const ownerNumber = ["6283146208804@s.whatsapp.net"]   // Ganti Nomor Lu
const pacarNumber = ["6283145932464@s.whatsapp.net"]  // Ganti Jadi Nomor Bot Lu
/************************************/
...
const antilinkyt = JSON.parse(fs.readFileSync('./database/kelompok/antilinkyt.json'))
const antivirtex = JSON.parse(fs.readFileSync('./database/kelompok/antivirtex.json'))
const antilinktele = JSON.parse(fs.readFileSync('./database/kelompok/antilinktele.json'))
...
client.version = [2,2143,3]
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('SCAN KODE QR NYA...','white'),color('Herman','lime'),color('Chanel','lime'),color('#go1ksubscribe','white'))
})

client.on('credentials-updated', () => {
	const authInfo = client.base64EncodedAuthInfo()
   console.log(`credentials updated!`)
   fs.writeFileSync('./hermanChanel.json', JSON.stringify(authInfo, null, '\t'))
})
fs.existsSync('./hermanChanel.json') && client.loadAuthInfo('./hermanChanel.json')
client.connect();
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*Byee...* @${num.split('@')[0]} 👋\n*Telah Meninggalkan Grup* ${mdata.subject}`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
thumbnail: fs.readFileSync('./Fake/herman.jpg'),
...
sellerJid: '0@s.whatsapp.net'
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("JAGA UCAPAN DONG!! 😠")
                        ...
                        .then(() => {
                            client.sendMessage(from, `*「 ANTI BADWORD 」*\n kamu akan dikick karena berkata kasar!`, text ,{quoted: mek})
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Link Yt Terdeteksi maaf ${sender.split("@")[0]} anda akan di kick dari group `)
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
	}
        ...
        if (messagesC.includes("*9999999*s *ผิดุท้เึางืผิดุท้เึางื* *9999999*")){
		if (!isGroup) return
		if (!isAntivirtex) return
		if (isGroupAdmins) return reply('Admin Mah Biarin')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
        ...
        if (messagesC.includes("https://t.me/")){
		if (!isGroup) return
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`telegram Terdeteksi maaf ${sender.split("@")[0]} anda akan di kick dari group `)
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
	}     
             //kolor
			...
			switch(command) {
                ...
                case 'delbadword':  // Herman Chanel
                    ...
                    break 
                case 'listbadword':  // Herman Chanel
                    ...
                    break 
				case 'getsticker':
				case 'gets':  // Herman Chanel
				
					namastc = body.slice(12)
					result = fs.readFileSync(`./strg/sticker/${namastc}.webp`)
					client.sendMessage(from, result, sticker, {quoted :faketroli})
					break
				case 'stickerlist':
				case 'liststicker':  // Herman Chanel
					...
					client.sendMessage(from, teks.trim(), extendedText, { quoted: faketroli, contextInfo: { "mentionedJid": setiker } })
					break
				case 'addsticker': // Herman Chanel
					...
					client.sendMessage(from, `Sukses Menambahkan Sticker\nCek dengan cara ${prefix}liststicker`, MessageType.text, { quoted: faketroli })
					break
				case 'addvn':  // Herman Chanel
...
quoted: mek
  ...
  break	
				case 'addimage':  // Herman Chanel
					...
					client.sendMessage(from, `Sukses Menambahkan Video\nCek dengan cara ${prefix}listimage`, MessageType.text, { quoted: faketroli })
					break
				case 'getimage':  // Herman Chanel
				...
				case 'getvideo':  
					...
					client.sendMessage(from, buffer, video, { mimetype: 'video/mp4', quoted: faketroli })
					break
				case 'listvideo':
				case 'videolist': 
		...
		case 'transfer':
              ...
              case 'soundplaydate':
				...
				client.sendMessage(from, XYTODmy, MessageType.audio, { quoted: faketroli, ptt: true })
				await limitAdd(sender)
				break
                case 'admin':
				if (!isOwner) return reply(ind.ownerb())
				...
				client.sendMessage(from, teks.trim(), extendedText, {quoted: faketroli, contextInfo: {"mentionedJid": adm}})
				break
                case 'premium':
				if (!isOwner) return reply(ind.ownerb())
				premm = body.slice(9)
		...
		case 'unpremium':
				...
				client.sendMessage(from, teks.trim(), extendedText, {quoted: faketroli, contextInfo: {"mentionedJid": prem}})
				break
                case 'ban':
				bnnd = body.slice(4)
		...
		case 'unban':
				...
				client.sendMessage(from, teks.trim(), extendedText, {quoted: faketroli, contextInfo: {"mentionedJid": ban}})
				break
		...
		case 'giftlimit': 
				...
				const nomerr = args[0].replace('@','')
                		...
                		if (!nomerr) return reply(`maaf format salah\nmasukan parameter yang benar\ncontoh : ${prefix}giftlimit @6283146208804 20`)
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				data = await fetchJson(`https://tobz-api.herokuapp.com/api/moddroid?q=${body.slice(10)}&apikey=RanddyGanz`)
				hepi = data.result[0] 
				teks = `*Nama*: ${data.result[0].title}\n*Publisher*: ${hepi.publisher}\n*Mod info:* ${hepi.mod_info}\n*size*: ${hepi.size}\n*Latest version*: ${hepi.latest_version}\n*Genre*: ${hepi.genre}\n*Link:* ${hepi.link}\n*Download*: ${hepi.download}`
				buffer = await getBuffer(hepi.image)
				client.sendMessage(from, buffer, image, {quoted: faketroli, caption: `${teks}`})
				await limitAdd(sender)
				break
		case 'happymod':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				data = await fetchJson(`https://tobz-api.herokuapp.com/api/happymod?q=${body.slice(10)}&apikey=RanddyGanz`)
				hupo = data.result[0] 
				teks = `*Nama*: ${data.result[0].title}\n*version*: ${hupo.version}\n*size:* ${hupo.size}\n*root*: ${hupo.root}\n*purchase*: ${hupo.price}\n*link*: ${hupo.link}\n*download*: ${hupo.download}`
				buffer = await getBuffer(hupo.image)
				client.sendMessage(from, buffer, image, {quoted: faketroliq, caption: `${teks}`})
				await limitAdd(sender)
				break
		case 'apkpure':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				data = await fetchJson(`https://api.zeks.xyz/api/apkpure?q=${body.slice(9)}&apikey=RanddyGanz`, {method: 'get'})
				teks = '=================\n'
		...
		case 'jadwalbola': 			
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				data = await fetchJson(`https://api.vhtear.com/jadwalbola&apikey=RanddyGanz`, {method: 'get'})
				teks = '=================\n'
		...
		case 'coronainfo': 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				data = await fetchJson(`https://api.vhtear.com/corona&apikey=RanddyGanz`, {method: 'get'})
				teks = '=================\n'
				for (let i of data.result) {
		...
		case 'bitly':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				client.updatePresence(from, Presence.composing) 
				data = await fetchJson(`https://tobz-api.herokuapp.com/api/bitly?url=${args[0]}&apikey=RanddyGanz`)
				hasil = `link : ${args[0]}\n\nOutput : ${data.result}`
				reply(hasil)
				await limitAdd(sender)
				break
		case 'cuttly':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				client.updatePresence(from, Presence.composing) 
				data = await fetchJson(`https://hujanapi.herokuapp.com/api/cuttly?url=${args[0]}&apikey=RanddyGanz`)
...
if (isBanned) return reply('```Lu kebanned kontol```')
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
				...
				reply('[❕] Loading') 
				buffer = await getBuffer(`https://lolhuman.herokuapp.com/api/textprome2/lionlogo?apikey=RanddyGanz&text1=${lion}&text2=${logo}`) 
				client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan Lupa Subrek YT BOT : https://youtube.com/channel/UCPhU6uA_3gUaEvyrFZFgJqg*', quoted: faketroli})
				await limitAdd(sender) 
				break
				case 'pornhub':
				var gh = body.slice(9) 
				...
				reply('[❕] Loading') 
				buffer = await getBuffer(`https://lolhuman.herokuapp.com/api/textprome2/pornhub?apikey=RanddyGanz&text1=${porn}&text2=${hub}`) 
				client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan Lupa Subrek YT BOT : https://youtube.com/channel/UCPhU6uA_3gUaEvyrFZFgJqg*', quoted: faketroli})
				await limitAdd(sender) 
				break
				case 'valorant':
				...
				reply('[❕] Loading') 
				buffer = await getBuffer(`https://lolhuman.herokuapp.com/api/ephoto3/valorantbanner?apikey=RanddyGanz&text1=${teks1}&text2=${teks2}&text3=${teks3}`) 
				client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan Lupa Subrek YT BOT : https://youtube.com/channel/UCPhU6uA_3gUaEvyrFZFgJqg*', quoted: faketroli})
				break
				case 'amongus':
...
if (isBanned) return reply('```Lu kebanned kontol```')
                    ...
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} herman`)
                    txt = args.join(" ")
                    reply('[❕] Loading')
                    buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/amongus?apikey=RanddyGanz&text=${txt}`)
                    client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan Lupa Subrek YT BOT : https://youtube.com/channel/UCPhU6uA_3gUaEvyrFZFgJqg*', quoted: faketroli})
         break
				...
				case 'googletext':
					...
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}googletext Herman/gans/abiezz`)
					reply(ind.wait())
		...
		case 'tinyurl':
               client.updatePresence(from, Presence.composing) 
                data = await fetchJson(`https://tobz-api.herokuapp.com/api/tinyurl?url=${args[0]}&apikey=RanddyGanz`)
                hasil = `link : ${args[0]}\n\nOutput : ${data.result}`
                reply(hasil)
                await limitAdd(sender)
                break
		case 'shrtco':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					exec(`wget ${anu.url} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						client.sendMessage(from, buffer, sticker, {quoted: faketroli})
						fs.unlinkSync(rano)
					})
					await limitAdd(sender)
					break
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson('https://tobz-api.herokuapp.com/api/nsfwblowjob?apikey=RanddyGanz', {method: 'get'})
					if (anu.error) return reply(anu.error)
		...
		case 'randomhentai':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					reply(ind.wait())
                                        ...
                                        client.sendMessage(from, buffer, image, {quoted: faketroli})
					await limitAdd(sender)
                                        break
		case 'cium':
		...
		case 'peluk':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson('https://api.shizukaa.xyz/api/randomimage?apikey=RanddyGanz', {method: 'get'})
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(ind.stikga())
		...
		case 'nulis':
				...
				ct = body.slice(6)
				reply(ind.wait())
				ct = await getBuffer(`https://api.zeks.xyz/api/nulis?text=${ct}&apikey=RanddyGanz`)
				client.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: faketroli})
				await limitAdd(sender)
				break
		case 'nulis2':
		...
		case 'galaxtext':
				...
				ct = body.slice(10)
				reply(ind.wait())
				ct = await getBuffer(`http://lolhuman.herokuapp.com/api/ephoto1/galaxywallpaper?apikey=RanddyGanz&text=${ct}`)
				client.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: faketroli})
				await limitAdd(sender)
				break
		case 'bplogo':
				...
				vinz = body.slice(8)
				reply(ind.wait())
				anu = await fetchJson(`https://api.zeks.xyz/api/logobp?text=${vinz}&apikey=RanddyGanz`)
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: faketroli})
				await limitAdd(sender)
				break
		case 'tahta':
         ...
         teks = args.join(" ") 
         reply('[❕] Loading') 
         buffer = await getBuffer(`https://api.zeks.xyz/api/hartatahta?text=${teks}&apikey=RanddyGanz`) 
         client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan Lupa Subrek YT BOT : https://youtube.com/channel/UCPhU6uA_3gUaEvyrFZFgJqg*', quoted: faketroli})
         break
		case 'darkneon':
                case 'candlemug':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} ${cr}`)
                    txt = args.join(" ")
                    reply('[â•] Loading')
                    buffer = await getBuffer(`https://videfikri.com/api/textmaker/${command}/?text=${txt}`)
                    client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan lupa follow ig instagram.com/cal_me_herman*', quoted: faketroli})
         break
         case 'wanted':
	if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
	...
	break
		case 'hartatata3':
				...
				bh = body.slice(12)
				reply(ind.wait())
				bh = await getBuffer(`https://api.vhtear.com/hartatahta?text=${bh}&apikey=RanddyGanz`)
				client.sendMessage(from, bh, image, {caption: 'Nih kak udah jadi..', quoted: faketroli})
				await limitAdd(sender)
				break
		case 'text2gif':
				...
				bh = body.slice(10)
				reply(ind.wait())
				bh = await getBuffer(`https://api.vhtear.com/textxgif?text=${bh}&apikey=RanddyGanz`)
				client.sendMessage(from, bh, image, {caption: 'Nih kak udah jadi..', quoted: faketroli})
				await limitAdd(sender)
				break
		case 'attp':
		...
		case 'ttp':
                    ...
                    txt = args.join(" ")
                    buffer = await getBuffer(`https://lolhuman.herokuapp.com/api/ttp3?apikey=RanddyGanz&text=${txt}`)
		...
		case 'thunder':
				...
				bhaine = body.slice(9)
				reply(ind.wait())
				anu = await fetchJson(`https://api.zeks.xyz/api/thundertext?text=${bhaine}&apikey=RanddyGanz`)
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: faketroli})
				await limitAdd(sender)
				break
		case 'imgmaker':
				...
				ainez = body.slice(10)
				reply(ind.wait())
				anu = await fetchJson(`http://lolhuman.herokuapp.com/api/quotemaker?apikey=RanddyGanz&text=${ainez}`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: faketroli})
				await limitAdd(sender)
				break
		case 'calendermaker':
				...
				reply(ind.wait())
				anu = await fetchJson(`https://api.zeks.xyz/api/calendar?img=${ainez2}&apikey=RanddyGanz`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: faketroli})
				await limitAdd(sender)
				break
		case 'nekonime':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					reply(ind.wait())
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/nekonime?apikey=RanddyGanz`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
		...
		case 'husbu':
				...
				reply(ind.wait())
				anu = await fetchJson(`https://tobz-api.herokuapp.com/api/husbu2?apikey=RanddyGanz`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {quoted: faketroli})
				await limitAdd(sender)
				break
		case 'shota':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				    try {
						res = await fetchJson(`https://tobz-api.herokuapp.com/api/randomshota?apikey=RanddyGanz`)
						buffer = await getBuffer(res.image)
						client.sendMessage(from, buffer, image, {quoted: faketroli, caption: 'Ingat! Cintai husbumu'})
					} catch (e) {
				...
				tels = body.slice(10)
				reply(ind.wait())
				anu = await fetchJson(`https://tobz-api.herokuapp.com/api/textpro?theme=jokerlogo&text=${tels}&apikey=RanddyGanz`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {quoted: faketroli})
				await limitAdd(sender)
				break
				...
				reply(ind.wait())
				anu = await fetchJson(`https://tobz-api.herokuapp.com/api/randomanime?apikey=RanddyGanz`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {quoted: faketroli})
				await limitAdd(sender)
				break
				...
				client.sendMessage(from, uhh, audio, {mimetype: 'audio/mp4', ptt:true, quoted: faketroli})
				fs.unlinkSync(ran)
				})
				break
				...
				case 'tupai':
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=0.5,asetrate=65100" ${ran}`, (err, stderr, stdout) => {
						...
						client.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: faketroli})
						fs.unlinkSync(ran)
					})
				break
				case 'gemok':
				...
				reply(ind.wait())
				buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/toloserti?apikey=RanddyGanz&name=${gatauda}`, {method: 'get'})
				client.sendMessage(from, buffer, image, {quoted: faketroli})
				await limitAdd(sender)
				break
				case 'tagme':
...
break
				...
				reply(ind.wait())
				buffer = await getBuffer(`https://api.zeks.xyz/api/emoji-image?apikey=RanddyGanz&emoji=${gatauda}`, {method: 'get'})
				client.sendMessage(from, buffer, image, {quoted: faketroli})
				await limitAdd(sender)
				break
				...
				gatauda = body.slice(8)
				reply(ind.wait())
				anu = await fetchJson(`https://api.zeks.xyz/api/memeindo?apikey=RanddyGanz`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {quoted: faketroli})
				await limitAdd(sender)
				break
         ...
         break 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				reply(ind.wait())
				anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/meme?apikey=RanddyGanz`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {quoted: faketroli})
				await limitAdd(sender)
				break
				...
				gatauda = body.slice(8)
				reply(ind.wait())
				buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/meme/darkjoke?apikey=RanddyGanz`, {method: 'get'})
				client.sendMessage(from, buffer, image, {quoted: faketroli})
				await limitAdd(sender)
				break
		case 'randomkpop':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				reply(ind.wait())
				anu = await fetchJson(`https://tobz-api.herokuapp.com/api/randomkpop?apikey=RanddyGanz`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {quoted: faketroli, caption: 'Nih kpopnya kak...'})
				await limitAdd(sender)
				break
		case 'aesthetic':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				reply(ind.wait())
				anu = await fetchJson(`https://api.zeks.xyz/api/estetikpic?apikey=RanddyGanz`, {method: 'get'})
				buffer = await getBuffer(anu.result.result)
				client.sendMessage(from, buffer, image, {quoted: faketroli, caption: 'Nih aestheticnya kak...'})
				await limitAdd(sender)
				break
		case 'logoepep': 
				gatauda = body.slice(9)
				...
				client.sendMessage(from, buffer, image, {quoted: faketroli, caption: 'Nih hasilnya kak...'})
				await limitAdd(sender)
				break
		case 'logoepep2': 
				gatauda = body.slice(10)
				...
				if (args.length < 1) return reply('Teksnya mana um')
				reply(ind.wait())
				buffer = await getBuffer(`https://api.vhtear.com/logoff?hero=Alok&text=${gatauda}&apikey=RanddyGanz`, {method: 'get'})
				client.sendMessage(from, buffer, image, {quoted: faketroli, caption: 'Nih hasilnya kak...'})
				await limitAdd(sender)
				break
		case 'logoepep3': 
				...
				if (args.length < 1) return reply('Teksnya mana um')
				reply(ind.wait())
				buffer = await getBuffer(`https://api.vhtear.com/logoff?hero=Alvaro&text=${gatauda}&apikey=RanddyGanz`, {method: 'get'})
				client.sendMessage(from, buffer, image, {quoted: faketroli, caption: 'Nih hasilnya kak...'})
				await limitAdd(sender)
				break
		case 'logoepep4': 
				...
				if (args.length < 1) return reply('Teksnya mana um')
				reply(ind.wait())
				buffer = await getBuffer(`https://api.vhtear.com/logoff?hero=Caroline&text=${gatauda}&apikey=RanddyGanz`, {method: 'get'})
				client.sendMessage(from, buffer, image, {quoted: faketroli, caption: 'Nih hasilnya kak...'})
				await limitAdd(sender)
				break
		case 'logoepep5': 
		...
		case 'gamelogo': 
				...
				if (args.length < 1) return reply('Teksnya mana um')
				reply(ind.wait())
				buffer = await getBuffer(`https://api.vhtear.com/gamelogo?text=${gatauda}&apikey=RanddyGanz`, {method: 'get'})
				client.sendMessage(from, buffer, image, {quoted: faketroli, caption: 'Nih hasilnya kak...'})
				await limitAdd(sender)
				break
		case 'pornlogo': 
				...
				if (args.length < 1) return reply('Teksnya mana um')
				reply(ind.wait())
				buffer = await getBuffer(`https://api.vhtear.com/pornlogo?text1=${gbl7}&text2=${gbl8}&apikey=RanddyGanz`, {method: 'get'})
				client.sendMessage(from, buffer, image, {quoted: faketroli, caption: 'Nih hasilnya kak...'})
				await limitAdd(sender)
				break
                case 'joox':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				data = await fetchJson(`https://tobz-api.herokuapp.com/api/joox?q=${body.slice(6)}&apikey=RanddyGanz`, {method: 'get'})
				if (data.error) return reply(data.error)
				infomp3 = `*Lagu Ditemukan!!!*\nJudul : ${data.result.judul}\nAlbum : ${data.result.album}\nDipublikasi : ${data.result.dipublikasi}`
				buffer = await getBuffer(data.result.thumb)
				lagu = await getBuffer(data.result.mp3)
				client.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
				client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${data.result.title}.mp3`, quoted: faketroli})
				await limitAdd(sender)
				break
		case 'play':   
				...
				reply(ind.wait())
				play = body.slice(5)
				anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp3?q=${play}&apikey=RanddyGanz`)
				if (anu.error) return reply(anu.error)
				infomp3 = `*Lagu Ditemukan!!!*\nJudul : ${anu.result.title}\nSource : ${anu.result.source}\nUkuran : ${anu.result.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM YA SAYANG*`
				buffer = await getBuffer(anu.result.thumbnail)
				client.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
				lagu = await getBuffer(anu.result.url_audio)
				client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: faketroli})
				await limitAdd(sender)
				break
		/*case 'play2':   
		...
		case 'vinta':
				...
				client.sendMessage(from, vintage, image, {caption: 'Nih kak ${vin}', quoted: faketroli})
				await limitAdd(sender)
				break
                case '1cak':
				    try {
                 ...
                 if (!isRegistered) return reply( ind.noregis())
						...
						client.sendMessage(from, buffer, image, {quoted: faketroli, caption: 'Nih Kak udah Jadi'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(ind.wrongf())
					}
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break	
                case 'faktaunik': // herman Chanel
					
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/faktaunik?apikey=RanddyGanz`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break		
		case 'quotes':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					data = await fetchJson(`https://api.shizukaa.xyz/api/randomquote?apikey=RanddyGanz`)
		...
		case 'covidglobal':    
				...
				client.updatePresence(from, Presence.composing) 
			    tels = body.slice(12)
				asu = await fetchJson(`http://lolhuman.herokuapp.com/api/corona/global?apikey=RanddyGanz`)
		...
		case 'ceknamaff': 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(11)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/freefire/${gatauda}?apikey=RanddyGanz`)
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'ceknamaml': 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(11)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/mobilelegend/${gatauda}?apikey=RanddyGanz`)
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'namaninja': 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(11)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/ninja?apikey=RanddyGanz&nama=${gatauda}`)
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'infonomor':
		...
		case 'infogempa':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/infogempa?apikey=RanddyGanz`, {method: 'get'})
					if (anu.error) return reply(anu.error)
  ...
  break
		case 'infocuaca':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/cuaca?wilayah=${tels}&apikey=RanddyGanz`, {method: 'get'})
					if (anu.error) return reply(anu.error)
		...
		case 'kurs': 
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`https://api.vhtear.com/kurs&apikey=RanddyGanz`, {method: 'get'})
					if (anu.error) return reply(anu.error)
		...
		case 'github': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`https://api.shizukaa.xyz/api/stalkgithub?apikey=RanddyGanz&user=${tels}`, {method: 'get'})
		...
		case 'tampar':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					client.updatePresence(from, Presence.composing) 
					data = await fetchJson(`https://api.zeks.xyz/api/tribunews?apikey=RanddyGanz`, {method: 'get'})
					teks = '=================\n'
					for (let i of data.result) {
						teks += `*Judul* : ${i.title}\n*Time* : ${i.time}\n*Link* : ${i.url}\n*Keterangan* : ${i.ket}\n=================\n`
					}
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					client.updatePresence(from, Presence.composing) 
					data = await fetchJson(`https://api.zeks.xyz/api/liputan6?apikey=RanddyGanz`, {method: 'get'})
					teks = '=================\n'
					for (let i of data.result) {
						teks += `*Judul* : ${i.title}\n*Url* : ${i.url}\n*Keterangan* : ${i.ket}\n*Category* : ${i.category}\n*Time* : ${i.time}\n=================\n`
					}
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					client.updatePresence(from, Presence.composing) 
					data = await fetchJson(`https://api.zeks.xyz/api/foxnews?apikey=RanddyGanz`, {method: 'get'})
					teks = '=================\n'
					for (let i of data.result) {
						teks += `*Judul* : ${i.title}\n*Url* : ${i.url}\n*Country* : ${i.country}\n*Content* : ${i.content}\n*Time* : ${i.time}\n=================\n`
					}
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					client.updatePresence(from, Presence.composing) 
					data = await fetchJson(`https://api.zeks.xyz/api/nickepep?apikey=RanddyGanz`, {method: 'get'})
					teks = '=================\n'
					for (let i of data.result) {
						teks += `*Nick* : ${i}\n=================\n`
					}
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					client.updatePresence(from, Presence.composing) 
					data = await fetchJson(`http://lolhuman.herokuapp.com/api/newsinfo?apikey=RanddyGanz`, {method: 'get'})
					teks = '=================\n'
					for (let i of data.result) {
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					client.updatePresence(from, Presence.composing) 
					data = await fetchJson(`https://api.zeks.xyz/api/film/2?q=${body.slice(6)}&apikey=RanddyGanz`, {method: 'get'})
					teks = '=================\n'
					for (let i of data.result) {
						teks += `*Title:* : ${i.title}\n*Url* : ${i.url}\n=================\n`
					}
                ...
                pshh = `${body.slice(12)}`
                  anu = await fetchJson(`https://api.zeks.xyz/api/film/2?q=${pshh}&apikey=RanddyGanz`, {method: 'get'})
                  puree = '======================\n'
		...
		case 'trendtwit':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					data = await fetchJson(`https://api.vhtear.com/trendtwitter?country=indonesia&apikey=RanddyGanz`, {method: 'get'})
					teks = '=================\n'
					for (let i of data.result) {
		...
		case 'pinterest':
           ...
           case 'glitchtext':
					...
					reply(ind.wait())
					buffer = await getBuffer(`https://api.zeks.xyz/api/gtext?text1=${gli}&text2=${tch}&apikey=RanddyGanz`)
					client.sendMessage(from, buffer, image, {quoted: mek})
					await limitAdd(sender)
				break
           case 'waifu':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.shizukaa.xyz/api/waifu?apikey=RanddyGanz`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
           case 'randombts':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.shizukaa.xyz/api/randombts?apikey=RanddyGanz`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
           case 'randomexo':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.shizukaa.xyz/api/randomexo?apikey=RanddyGanz`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
           case 'blackpink':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.shizukaa.xyz/api/blackpink?apikey=RanddyGanz`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
           case 'imageislamic':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.shizukaa.xyz/api/wpislamic?apikey=RanddyGanz`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
           case 'imagecyberspace':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.shizukaa.xyz/api/wpcyberspace?apikey=RanddyGanz`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
           case 'imagegame':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.shizukaa.xyz/api/wpgame?apikey=RanddyGanz`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
		     ...
		     case 'fakta':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.shizukaa.xyz/api/fakta?apikey=RanddyGanz`, {method: 'get'})
				...
				case 'katabijak':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.shizukaa.xyz/api/bijak?apikey=RanddyGanz`, {method: 'get'})
			...
			case 'katailham':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				anu = await fetchJson(`https://api.shizukaa.xyz/api/bacotanilham?apikey=RanddyGanz`, {method: 'get'})
				kata = anu.result
...
quoted: mek, caption: `Gimana? *Jangan Lupa Follow IG Bot : instagram.com/gustinx7`
  })
  } catch {
    reply(mess.ferr)
  }
...
quoted: mek, caption: `Wah ganteng kek gua`
  ...
  break
				...
				case 'tebakgambar':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api-mikubot.herokuapp.com/api/kuis/tebakgambar?apikey=RanddyGanz`, {method: 'get'})
		...
		case 'spamcall':
			...
			break  
                ...
                reply(mess.wait)
                wiki = `${body.slice(6)}`
                anu = await fetchJson(`https://api.shizukaa.xyz/api/wiki?apikey=RanddyGanz&q=${wiki}`, {method: 'get'})
                if (anu.error) return reply(anu.error)
                wikii = `${anu.result}`
               ...
               case 'asupan':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                reply(mess.wait)
                anu = await fetchJson(`https://api.shizukaa.xyz/api/asupan?apikey=RanddyGanz`)
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned astagfirullah!')
                    ...
                    txt = args.join(" ")
                    reply('[❕] Loading')
                    buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/carbon?apikey=RanddyGanz&code=${txt}`)
                    client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan Lupa  Subrek YT BOT : https://youtube.com/channel/UCMT6vvdujN62xToLZeW2FSA*', quoted: mek})
         break
         ...
         teks = args.join(" ") 
         reply('[❕] Loading') 
         buffer = await getBuffer(`https://lolhuman.herokuapp.com/api/photooxy3/bannerlol?apikey=RanddyGanz&text=${teks}`) 
         client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan Lupa  Subrek YT BOT : https://youtube.com/channel/UCMT6vvdujN62xToLZeW2FSA*', quoted: mek})
         break 
         ...
         teks = args.join(" ") 
         reply('[❕] Loading') 
         buffer = await getBuffer(`https://lolhuman.herokuapp.com/api/textprome/text1917?apikey=RanddyGanz&text=${teks}`) 
         client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan Lupa  Subrek YT BOT : https://youtube.com/channel/UCMT6vvdujN62xToLZeW2FSA*', quoted: mek})
         break
                case 'pinterest':
					 // Miku Bot
...
if (isBanned) return reply('```Lu kebanned kontol```')
                    ...
                    ini_url = args[0]
                    ini_url = await fetchJson(`http://api.lolhuman.xyz/api/instagram?apikey=RanddyGanz&url=${ini_url}`)
               ...
               case 'ytmp4':
                    ini_link = args[0]
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/ytvideo?apikey=RanddyGanz&url=${ini_link}`)
...
if (isBanned) return reply('```Lu kebanned kontol```')
                    ...
                    query = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/xnxxsearch?apikey=RanddyGanz&query=${query}`)
                    get_result = get_result.result
                    txt = ""
			 ...
			 case 'nsfwloli':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
						...
						res = await fetchJson(`https://api.shizukaa.xyz/api/neko?apikey=RanddyGanz`, {method: 'get'})
		...
		case 'hilih': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					anu = await fetchJson(`https://api.zeks.xyz/api/hilihmaker?text=${body.slice(7)}&apikey=RanddyGanz`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'holoh': 
		...
		case 'lirik': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					teks = body.slice(6)
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/lirik?q=${teks}&apikey=RanddyGanz`, {method: 'get'})
		...
		case 'alay':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('kasih teks lah^_^!!!')
					data = await fetchJson(`https://api.zeks.xyz/api/alaymaker?kata=${body.slice(6)}&apikey=RanddyGanz`)
					reply(data.result)
				        await limitAdd(sender)
					break
		case 'alay2': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('kasih teks lah^_^!!!')
					data = await fetchJson(`http://lolhuman.herokuapp.com/api/upperlower?apikey=RanddyGanz&text=${body.slice(6)}`)
					reply(data.result)
				        await limitAdd(sender)
					break
		case 'fml':	// Herman Chanel
					
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					data = await fetchJson(`https://tobz-api.herokuapp.com/api/randomfmylife?apikey=RanddyGanz`)
					hasil = data.result
					reply(hasil)
					await limitAdd(sender)
					break	
		case 'chord':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					tels = body.slice(7)					
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/chord?q=${tels}&apikey=RanddyGanz`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'katacinta':
		...
		case 'pantun':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					gatauda = body.slice(8)					
					anu = await fetchJson(`https://api.zeks.xyz/api/pantun?apikey=RanddyGanz`, {method: 'get'})
		...
		case 'cersex': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					gatauda = body.slice(8)					
					anu = await fetchJson(`https://api.vhtear.com/cerita_sex&apikey=RanddyGanz`, {method: 'get'})
		...
		case 'jadwaltv':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					gatauda = body.slice(10)					
					anu = await fetchJson(`https://api.zeks.xyz/api/jadwaltv?channel=${gatauda}&apikey=RanddyGanz`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'katadilan': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`https://api.shizukaa.xyz/api/bacotandilan?apikey=RanddyGanz`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'katabucin': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`https://api.shizukaa.xyz/api/bucin?apikey=RanddyGanz`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'katabucin2': 
		...
		case 'cerpen': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/cerpen?apikey=RanddyGanz`, {method: 'get'})
		...
		case 'quotes': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/quotes?apikey=RanddyGanz`, {method: 'get'})
		...
		case 'bacotanhacker': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`https://api.shizukaa.xyz/api/bacotanhacker?apikey=RanddyGanz`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'quotedoraemon': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`https://api.shizukaa.xyz/api/quotedoraemon?apikey=RanddyGanz`, {method: 'get'})
					reply(anu.result.quote)
					await limitAdd(sender)
					break
		case 'resepmasakan':
		...
		case 'igstalk':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					hmm = await fetchJson(`https://api.zeks.xyz/api/igstalk?username=${body.slice(9)}&apikey=RanddyGanz`)
		...
		case 'infofilm': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					if (isBanned) return reply('Maaf kamu sudah terbenned!')
					ige = body.slice(10)
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/film2?q=${ige}&apikey=RanddyGanz`)
		...
		case 'infofilm2': 
                 if (!isRegistered) return reply( ind.noregis())
					...
					if (isBanned) return reply('Maaf kamu sudah terbenned!')
					ige = body.slice(10)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/lk21?apikey=RanddyGanz&query=${ige}`)
					buffer7 = await getBuffer(anu.result.thumbnail)
		...
		case 'heroml':  
				...
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
					 ige = body.slice(9)
                     anu = await fetchJson(`http://lolhuman.herokuapp.com/api/instagram?apikey=RanddyGanz&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
				...
				if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(mess.error.lv)
					 ige = body.slice(12)
                     anu = await fetchJson(`http://lolhuman.herokuapp.com/api/tiktok?apikey=RanddyGanz&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				 ...
				 case 'ceritahorror': 
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                   reply(`_[❕] Loading_`)
				   anu = await fetchJson(`https://naufalhoster.xyz/tools/story_horror?apikey=RanddyGanz`, {method:'get'})
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group Gusti-BOT :\n\n\`\`\``
					no = 0
		...
		case 'menu':
            //Gk Ush Di Hapus Tod, Dh Susah" Ngefix Di Hapus Sofun kh bgitu?:v
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    ...
                    shiz = `◪ INFO DEVELOPER
☕Nama : HermanSyah >3
📌WhatsApp : Wa.me/6283146208804
🛑YouTube : https://bit.ly/3c2q2Oy
...
► ${prefix}antilink [1/0]
► ${prefix}antilinktele [1/0]
► ${prefix}antilinkyt [1/0]
► ${prefix}antilinkig [1/0]
► ${prefix}leveling [1/0]
...
Spam call,Spam command = *BLOCK + BANNED*` 
                    ...
                    client.sendMessage(from, wew, image, { quoted: faketroli, caption: shiz })
					break
		...
		case 'pictmenu':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					wew = fs.readFileSync(`./src/logo.jpg`)
					client.sendMessage(from, wew, image, { quoted: faketroli, caption: randompict(prefix)})
					break
		case 'menuowner':
		case 'ownermenu':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					wew = fs.readFileSync(`./src/logo.jpg`)
					client.sendMessage(from, wew, image, { quoted: faketroli, caption: developer(prefix)})
					break
		case 'bahasa':
...
tels = body.slice(6)
try {
anu = await fetchJson(`https://tobz-api.herokuapp.com/api/wiki?q=${tels}&apikey=RanddyGanz`, {
 method: 'get'
}
)
reply(anu.result
...
caption: teks, contextInfo: {
  ...
  break
		...
		case 'lpr':
					...
					client.sendMessage('6283146208804@s.whatsapp.net', options, text, {quoted: faketroli})
					reply('Masalah telah di laporkan ke owner , Laporan palsu atau main² tidak akan ditanggapi.')
					break
					
		case 'request':
					const cfrr = body.slice(8)
					if (cfrr.length > 300) return client.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: faketroli})
					var nomor = mek.participant
		...
		case 'blocklist': 
					...
					client.sendMessage(from, teks.trim(), extendedText, {quoted: faketroli, contextInfo: {"mentionedJid": blocked}})
					break
                case 'hidetag':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                                ...
                                const tagzz = {
                                        ...
                                        client.sendMessage(from, tagzz, text, {quoted: faketroli})
					await limitAdd(sender)
                                        break
                                        case 'profile':
					...
					client.sendMessage(from, buffer, image, {quoted: faketroli, caption: profile})
					break
                case 'quotemaker':
		...
		case 'dare':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, tod, image, { quoted: faketroli, caption: '*Dare*\n\n'+ der })
					await limitAdd(sender)
					break
		case 'quoteskehidupan':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, crot, image, { quoted: faketroli, caption: '*Quotes Kehidupan*\n\n'+ quo })
					await limitAdd(sender)
					break
		case 'quotesislami':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, islam, image, { quoted: faketroli, caption: '*Quotes Islami*\n\n'+ isl })
					await limitAdd(sender)
					break	
		case 'quotesnasehat':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, nase, image, { quoted: faketroli, caption: '*Quotes Nasehat*\n\n'+ nsh })
					await limitAdd(sender)
					break	
		case 'quotescinta':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, cta, image, { quoted: faketroli, caption: '*Quotes Cinta*\n\n'+ cin })
					await limitAdd(sender)
					break
		case 'quotesmotivasi':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, vass, image, { quoted: faketroli, caption: '*Motivasi*\n\n'+ cin })
					await limitAdd(sender)
					break
                case 'fotocewek':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
	                                const cangti = ['https://i.ibb.co/1T1DCz7/cewek-thailand-20200325-007-non-fotografer-kly.jpg','https://i.ibb.co/FsJ6jjs/kucing2.jpg','https://i.ibb.co/vvkdS7n/kucing3.jpg','https://i.ibb.co/1QHWxts/kucing4.jpg','https://i.ibb.co/JQmRz4n/kucing5.jpg','https://i.ibb.co/tBwrFkG/kucing6.jpg','https://i.ibb.co/dp0YhYm/kucing7.jpg','https://i.ibb.co/R03smZT/kucing8.jpg','https://i.ibb.co/17tw0dp/kucing9.jpg','https://i.ibb.co/7XdGGqc/kucing10.jpg','https://i.ibb.co/XL9PZxg/kucing11.jpg','https://i.ibb.co/gyjvXWN/kucing12.jpg','https://i.ibb.co/R4gg4wH/kucing13.jpg','https://i.ibb.co/PmLYtFm/kucing14.jpg','https://i.ibb.co/XbSDh47/kucing15.jpg','https://i.ibb.co/kSXNJzt/kucing16.jpg']
                                        ...
                                        client.sendMessage(from, cangtip, image, {quote: faketroli})
					await limitAdd(sender)
                                        break
                /*case 'kucing':
		...
		case 'loli':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                                        gatauda = body.slice(6)
					reply(ind.wait())
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/randomloli?apikey=RanddyGanz`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: faketroli})
					await limitAdd(sender)
                                        break
		case 'waifu':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, hasil, image, {quoted: faketroli})
					await limitAdd(sender)
					break
		case 'loli2':
		...
		case 'animecry':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson('https://tobz-api.herokuapp.com/api/cry?apikey=RanddyGanz', {method: 'get'})
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(mess.error.stick)
		...
		case 'animehug':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson('https://tobz-api.herokuapp.com/api/hug?apikey=RanddyGanz', {method: 'get'})
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(mess.error.stick)
		...
		case 'pokemon':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					pok = await getBuffer(nimek)
					client.sendMessage(from, pok, image, { quoted: faketroli })
					await limitAdd(sender)
					break
				case 'freefireimg':
		...
		case 'kelinci':
				...
				if (!isNsfw) return reply(ind.nsfwoff())
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson('https://tobz-api.herokuapp.com/api/kiss?apikey=RanddyGanz', {method: 'get'})
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(ind.stikga())
				...
				if (!isNsfw) return reply(ind.nsfwoff())
				    try {
						res = await fetchJson(`https://tobz-api.herokuapp.com/api/husbu?apikey=RanddyGanz`)
						buffer = await getBuffer(res.image)
    ...
    case 'linkgc':
        ...
        reply('https://chat.whatsapp.com/'+linkgc)
                    break
		case 'sayang':
					client.sendMessage(from, 'Apa sayangku? 🥰',MessageType.text, { quoted: faketroli} )
					break
		case 'gelud':
					client.sendMessage(from, 'Kuylah, dapat duit aku..',MessageType.text, { quoted: faketroli} )
					break
		case 'bot':
					client.sendMessage(from, 'Iya kak? \nAda yang bsa saya bantu?"',MessageType.text, { quoted: faketroli} )
					break
		case 'hargaprem':
		case 'hargapremium':
					client.sendMessage(from, 'Level 100 = Free 3 day (premium)\n10k = 30day (premium)',MessageType.text, { quoted: faketroli} )
					break
		case 'buypremium':
					client.sendMessage(from, 'Ingin membeli premium?\nHarap hubungi kami : wa.me/6283146208804',MessageType.text, { quoted: faketroli} )
					break
		case 'buypremunlimited':
					
					client.sendMessage(from, 'Selamat kamu sudah berlangganan selamanya 🎉',MessageType.text, { quoted: faketroli} )
					break
		case 'buyprem30day':
					
					client.sendMessage(from, 'Selamat kamu sudah berlangganan 30 hari',MessageType.text, { quoted: faketroli} )
					break
		case 'buyprem7day':
					
					client.sendMessage(from, 'Selamat kamu sudah berlangganan 7 hari',MessageType.text, { quoted: faketroli} )
					break
		case 'test':
					client.sendMessage(from, 'Active',MessageType.text, { quoted: faketroli} )
					break
		case 'ping':
					client.sendMessage(from, 'Active',MessageType.text, { quoted: faketroli} )
					break
		case 'assalamualaikum':
					client.sendMessage(from, 'Waalaikumusalam',MessageType.text, { quoted: faketroli} )
					break
		case 'kontol':
					client.sendMessage(from, 'Gunakan bahasa yang benar\nAnda akan kami banned!\nHubungi kami : wa.me/6283146208804',MessageType.text, { quoted: faketroli} )
					break
		...
		case 'wame':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
  					...
  					client.updatePresence(from, Presence.composing) 
  					options = {
  					text :`「 *${cr} BOT WHATSAPP* 」\n\n_Request by_ : *@${sender.split("@s.whatsapp.net")[0]}\n\nYour link WhatsApp : *https://wa.me/${sender.split("@s.whatsapp.net")[0]}*\n*Or ( / )*\n*https://api.whatsapp.com/send?phone=${sender.split("@")[0]}*`,
  					contextInfo: { mentionedJid: [sender] }
  					}
  					client.sendMessage(from, options, text, { quoted: faketroli } )
  					break
  					if (data.error) return reply(data.error)
  					reply(data.result)
					await limitAdd(sender)
		...
		case 'bc': 
					...
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : faketroli
		...
		case 'bcc': 
				if (!isAdmin) return reply('*Only Admin bot*')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST ${namebot} 」*\n\n${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
		...
		case 'group':
            ...
            case 'developer':
                  ...
                  client.sendMessage(from, 'Tuh nomer owner ku >3',MessageType.text, { quoted: mek} )
					break    
           /*case 'setname':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
...
case 'simi': 
		...
		case 'welcome':
                 ...
                 case 'antilink':
					...
					if (args.length < 1) return reply('ketik 1 untuk mengaktifkan')
					if (Number(args[0]) === 1) {
						if (isAntilinkyt) return reply('UDAH NYALA KAK')
						antilinkyt.push(from)
						fs.writeFileSync('./database/kelompok/antilinkyt.json', JSON.stringify(antilinkyt))
						reply('SUKSES MENGAKTIFKAN ANTI LINK YT DI GROUP')
						client.sendMessage(from,`ALLERT!!! Jika bukan admin jangan kirim link grup`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntilinkyt) return reply('EMANG AKTIF?')
						var ini = anti.botLangsexOf(from)
						antilinkyt.splice(ini, 1)
						fs.writeFileSync('./database/kelompok/antilinkyt.json', JSON.stringify(antilinkyt))
						reply('SUKSES MEMATIKAN ANTI LINK YT DI GROUP')
					} else {
						reply('1 untuk mengaktifkan, 0 untuk menonaktifkan')
					}
					break
					...
					if (Number(args[0]) === 1) {
						...
						reply('SUKSES MENGAKTIFKAN ANTI LINK IG DI GROUP')
						client.sendMessage(from,`ALLERT!!! Jika bukan admin jangan kirim link grup`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntilinkig) return reply('EMANG AKTIF?')
						var ini = anti.botLangsexOf(from)
					...
					if (args.length < 1) return reply('ketik 1 untuk mengaktifkan')
					if (Number(args[0]) === 1) {
						if (isAntilinktele) return reply('UDAH NYALA KAK')
						antilinktele.push(from)
						fs.writeFileSync('./database/kelompok/antilinktele.json', JSON.stringify(antilinktele))
						reply('SUKSES MENGAKTIFKAN ANTI LINK TELE DI GROUP')
						client.sendMessage(from,`ALLERT!!! Jika bukan admin jangan kirim link grup`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntilinktele) return reply('EMANG AKTIF?')
						var ini = anti.botLangsexOf(from)
						antilinktele.splice(ini, 1)
						fs.writeFileSync('./database/kelompok/antilinktele.json', JSON.stringify(antilinktele))
						reply('SUKSES MEMATIKAN ANTI LINK TELE DI GROUP')
					} else {
						reply('1 untuk mengaktifkan, 0 untuk menonaktifkan')
					}
					break
